...
Function Get-ItGlueJsonWebToken {
    ...
    If ($OTP -match '.+') {$attributes | Add-Member -Type NoteProperty -Name "otp_attempt" -Value $OTP}
    ...
    Try {
        ...
        Write-Debug "Token Resulting:"
        Write-Debug $refreshToken
    }
    Catch {
        Write-Debug "Token Result: $($refreshToken|ConvertTo-JSON -Depth 5 -Compress)"
...
Function Get-ItGlueOrganization {
    ...
    Else {
        ...
        If (!$ItGlueApiKey) {
            ...
            $header.add('authorization', "Bearer $([System.Runtime.InteropServices.Marshal]::PtrToStringAuto([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($ItGlueApiKey)))")
        }
    }
...
Function Get-ItGlueOrganizationPasswords {
    <#
        .DESCRIPTION
            Gets all matching instances of passwords for an Organization.
        .NOTES
            V1.0.0.0 date: 24 July 2019
                - Initial release.
        .PARAMETER ItGlueApiKey
            ITGlue API key used to send data to ITGlue.
        .PARAMETER ItGlueUserCred
        ...
        .PARAMETER BlockLogging
            When this switch is included, the code will write output only to the host and will not attempt to write to the Event Log.
        .EXAMPLE
            PS C:\> Get-ItGlueOrganizationPasswords -ItGlueApiKey ITG.XXXXXXXXXXXXX -OrganizationID 123456

            In this example, the cmdlet will get all instances passwords for Organization ID 123456, using the provided ITGlue API key. Output will be sent to the host session and to the Windows event log.
        .EXAMPLE
            PS C:\> Get-ItGlueOrganizationPasswords -OrganizationID 123456 -ItGlueUserCred (Get-Credential) -BlockLogging -Verbose
    ...
    Else {
        ...
        If (!$ItGlueApiKey) {
            ...
            $ItGlueApiHeader.add('authorization', "Bearer $([System.Runtime.InteropServices.Marshal]::PtrToStringAuto([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($ItGlueApiKey)))")
        }
    }

    $message = ("Attempting to retrieve all the requested passwords from ITGlue.")
    If (($BlockLogging) -AND ($PSBoundParameters['Verbose'])) { Write-Verbose $message } ElseIf ($PSBoundParameters['Verbose']) { Write-Verbose $message; Write-EventLog -LogName Application -Source $EventLogSource -EntryType Information -Message $message -EventId 5417 }
    ...
    Do {
        Try {
            ...
            $allExistingPasswords = Invoke-RestMethod -Method GET -Headers $ItGlueApiHeader -Uri "$ItGlueUriBase/organizations/$OrganizationId/relationships/passwords" -Body $adQueryBody -ErrorAction Stop
        ...
        Catch {
            ...
            Else {
                $message = ("{0}: Unexpected error getting passwords. To prevent errors, {1} will exit. PowerShell returned: {2}" -f [datetime]::Now, $MyInvocation.MyCommand, $_.Exception.Message)
                If ($BlockLogging) { Write-Error $message } Else { Write-Error $message; Write-EventLog -LogName Application -Source $EventLogSource -EntryType Error -Message $message -EventId 5417 }
    ...
    $loopCount = 0
    $stopLoop = $false
    $totalpages=$allExistingPasswords.meta.'total-pages'
    $allExistingPasswords = $allExistingPasswords.data
    $allExistingPasswords += for ($i = 2; $i -le $($totalpages); $i++) {
        $adQueryBody = @{
            "page[size]"                     = $ItGluePageSize
            "page[number]"                   = $i
            "sort"                           = "name"
        ...
        $message = ("Getting page {0} of {1} of the requested flexible assets." -f $i, $totalpages)
        If (($BlockLogging) -AND ($PSBoundParameters['Verbose'])) { Write-Verbose $message } ElseIf ($PSBoundParameters['Verbose']) { Write-Verbose $message; Write-EventLog -LogName Application -Source $EventLogSource -EntryType Information -Message $message -EventId 5417 }
        ...
        Do {
            Try {
                ...
                (Invoke-RestMethod -Method GET -Headers $ItGlueApiHeader -Uri "$ItGlueUriBase/organizations/$OrganizationId/relationships/passwords" -Body $adQueryBody -ErrorAction Stop).data
    ...
    Return $allExistingPasswords
} #1.0.0.6
Function Get-ItGluePasswords {
    <#
        .DESCRIPTION
            Gets all matching instances of passwords.
        .NOTES
            V1.0.0.0 date: 24 July 2019
                - Initial release.
        .PARAMETER ItGlueApiKey
        ...
        .PARAMETER BlockLogging
            When this switch is included, the code will write output only to the host and will not attempt to write to the Event Log.
        .EXAMPLE
            PS C:\> Get-ItGluePasswords -ItGlueApiKey ITG.XXXXXXXXXXXXX -PasswordID 123456

            In this example, the cmdlet will get all instances passwords for Organization ID 123456, using the provided ITGlue API key. Output will be sent to the host session and to the Windows event log.
        .EXAMPLE
            PS C:\> Get-ItGluePasswords -OrganizationID 123456 -ItGlueUserCred (Get-Credential) -BlockLogging -Verbose
    ...
    param (
        [Parameter(ParameterSetName = 'ITGlueApiKey', Mandatory)]
        [Parameter(ParameterSetName = 'ITGluePasswordAndAPI', Mandatory)]
        [SecureString]$ItGlueApiKey,

        [Parameter(ParameterSetName = 'ITGlueUserCred', Mandatory)]
        [Parameter(ParameterSetName = 'ITGluePasswordAndCred', Mandatory)]
        [System.Management.Automation.PSCredential]$ItGlueUserCred,

        [Parameter(ParameterSetName = 'ITGluePasswordAndCred')]
        [Parameter(ParameterSetName = 'ITGluePasswordAndAPI')]
        [Nullable[Int64]]$PasswordId = $null,

        [Parameter(ParameterSetName = 'ITGlueApiKey')]
        [Parameter(ParameterSetName = 'ITGlueUserCred')]
    ...
    $x_URI="$ItGlueUriBase/passwords"
    If ($PasswordId -gt 0) {$x_URI=$x_URI+"/$PasswordId"}
    $loopCount = 0
    Do {
        Try {
            $loopCount++
            ...
            If ($OrganizationId) {$adQueryBody.Add("filter[organization-id]",$OrganizationId)}
            If ($ShowPassword) {$adQueryBody.Add("show_password",$ShowPassword)}
    
            $allExistingPasswords = Invoke-RestMethod -Method GET -Headers $ItGlueApiHeader -Uri $x_URI -Body $adQueryBody -ErrorAction Stop
    ...
    $allExistingPasswords += for ($i = 2; $i -le $($totalpages); $i++) {
        ...
        $message = ("Getting page {0} of {1} of the requested password assets." -f $i, $totalpages)
        If (($BlockLogging) -AND ($PSBoundParameters['Verbose'])) { Write-Verbose $message } ElseIf ($PSBoundParameters['Verbose']) { Write-Verbose $message; Write-EventLog -LogName Application -Source $EventLogSource -EntryType Information -Message $message -EventId 5417 }
        ...
        Do {
            Try {
                ...
                (Invoke-RestMethod -Method GET -Headers $ItGlueApiHeader -Uri $x_URI -Body $adQueryBody -ErrorAction Stop).data
